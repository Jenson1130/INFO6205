...
public class BSTSimple<Key extends Comparable<Key>, Value> implements BSTdetail<Key, Value> {
    ...
    public int maximumDepth(Node node){
        if (node == null)
            ...
            int lDepth = maximumDepth(node.smaller);
            int rDepth = maximumDepth(node.larger);
    ...
    public double averageDepth(Node node){
        double averageDepth = 0;
        if (node == null)
            return 0;
        else{
            double total_depth = (double) totalDepth(node);
            averageDepth = (total_depth - node.count) / node.count;
            return averageDepth;
        }
